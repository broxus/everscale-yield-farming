...
describe('Test Ton Farm Pool - upgrade to V3', async function() {
    ...
    describe('Upgrade staking pipeline testing', async function() {
        describe('Farm pool setup', async function() {
            it('Deploy fabric contract', async function () {
                fabric = await setupFabric(admin_user, 1, 1, 1);
            });
            ...
            it('Deploy farm pool contract', async function() {
                ...
                farm_pool = await fabric.deployPool({
                    ...
                    reward_rounds: [{startTime: farmStart, rewardPerSecond: [rewardPerSec]}],
                ...
                [farm_pool_reward_wallet_1] = await farm_pool.rewardWallets();
            });
        ...
        describe('Imitate migration', async function () {
            ...
            describe('1st user start farming', async function() {
                ...
                it('Deposit 2nd time', async function() {
                    ...
                    const tx = await farm_pool.deposit(userTokenWallet1, minDeposit, userTokenWallet1._owner, {compute: [null, 60]});
                    await afterRun();
                    await checkTokenBalances(
            ...
            describe('Updating all farming pools', async function() {
                it('Install new codes', async function() {
                    const pool_version_fabric_old = await fabric.pool_version();
                    expect(pool_version_fabric_old.toString()).to.be.eq('0', 'Pool version not updated');

                    const poolV3 = await locklift.factory.getContract('EverFarmPoolV3');
                    const tx = await fabric.installNewFarmPoolCode(poolV3.code);
                ...
                it('Upgrade pool', async function() {
                    const pool_version_old = await farm_pool.version();
                    expect(pool_version_old.toString()).to.be.eq('0', 'Pool version not updated');

                    const old_details = await farm_pool.details();

                    const poolV3 = await locklift.factory.getContract('EverFarmPoolV3');
                    await locklift.tracing.addToContext(farm_pool.address, poolV3);
                    ...
                    // up to new abi
                    farm_pool = await FarmPool.from_addr(farm_pool.address, farm_pool.owner, 'EverFarmPoolV3');
                ...
                it('Force claim user data', async function() {
                    userData1 = await farm_pool.userData(user1);
                    const old_details = await getUserDataDetails(userData1);
                    console.log('Here');

                    const tx = await farm_pool.claimRewardForUser(user2, user1, {compute: [60]});
                    await afterRun();
                });
            });
            ...
            describe('1st user continue farming', async function() {
                it('User withdraw half of staked amount', async function() {
                    ...
                    const tx = await farm_pool.withdrawTokens(user1, minDeposit, {compute: [60]});
                    await afterRun();
                    await checkTokenBalances(
                        userTokenWallet1, minDeposit, minDeposit, userInitialTokenBal - minDeposit
                    );
                ...
                it('User withdraw other half', async function() {
                    ...
                    const claim_tx = await farm_pool.claimReward(user1, {compute: [60]});
                    const new_reward_time = await farm_pool.lastRewardTime();
            ...
            describe('2nd user start farming (user data last version already)', async function() {
                it('Deposit tokens', async function() {
                    const tx = await farm_pool.deposit(userTokenWallet2, minDeposit, userTokenWallet2._owner, {compute: [60, null]});
                    await afterRun();
                    await checkTokenBalances(
                    ...
                    expect(details.current_version.toString()).to.be.eq('0', 'Bad user data version');
                });
                ...
                it('Deposit 2nd time', async function() {
                    ...
                    const tx = await farm_pool.deposit(userTokenWallet2, minDeposit, userTokenWallet2._owner, {compute: [60]});
                    await afterRun();
                    await checkTokenBalances(
                ...
                it('User withdraw half of staked amount', async function() {
                    ...
                    const tx = await farm_pool.withdrawTokens(user2, minDeposit, {compute: [60]});
                    await checkTokenBalances(
                        userTokenWallet2, minDeposit, minDeposit, userInitialTokenBal - minDeposit
                    );
                ...
                it('User withdraw other half', async function() {
                    ...
                    // check claim reward func
                    const claim_tx = await farm_pool.claimReward(user2, {compute: [60]});
                    const new_reward_time = await farm_pool.lastRewardTime();
                    await afterRun();
